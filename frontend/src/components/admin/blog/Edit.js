...
const Edit = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/api/admin/blog/show/" + params.id)
      .then((res) => {
        if (res.data.errors) {
  ...
  const handlSubmit = (e) => {
    ...
    if (!flag) {
      ...
      axios
        .put(
          "http://localhost:3000/api/admin/blog/edit/" + params.id,
          formData,
          {
  ...
  return (
    <div className="container mx-auto max-w-[500px] px-[15px]">
      ...
      >
        <h2 className=" font-bold uppercase mb-3 text-[20px]">Edit Blog</h2>
        <div className="mb-4">
          <input
