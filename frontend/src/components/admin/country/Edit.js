...
const Edit = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/api/admin/country/show/" + params.id)
      .then((res) => {
        if (res.data.errors) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!flag) {
      ...
      axios
        .put("http://localhost:3000/api/admin/country/edit/" + params.id, data)
        .then((res) => {
          if (res.data.errors) {
            console.log(res.data.errors);
          } else {
  ...
  return (
    <div className="container mx-auto max-w-[500px] px-[15px]">
      ...
      <form className=" shadow-2xl p-[20px]" onSubmit={handleSubmit}>
        <h2 className=" font-bold uppercase mb-3 text-[20px]">Edit Country</h2>
        <div className="mb-4">
          <input
            name="name"
